...
import { goToApprovePurchasePage, goToHomePage, goToProfilePage, goToRegisterPage } from "../routes/coordinator";
...
export const loginProfile = (body, clear, navigate) => {
  axios
    .post(`${BASE_URL}/login`, body)
    .then((res) => {
      localStorage.setItem("token", res.data.token);
...
export const loginApprove = (body, clear, navigate) => {
  axios
    .post(`${BASE_URL}/login`, body)
    .then((res) => {
      localStorage.setItem("token", res.data.token);
